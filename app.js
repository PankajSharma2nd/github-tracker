// app.js - Client-side code for GitHub Pages

// Configuration
const SERVER_URL = 'https://githubpages-sw-server.onrender.com';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.register('service-worker.js', {
            scope: '/' // Try to request maximum scope
        })
        .then(registration => {
...
window.addEventListener('beforeunload', () => {
    ...
    // We won't call logNavigationToServer here because the request might not complete
});
...
// Log any clicks on links
document.addEventListener('click', (event) => {
    if (event.target.tagName === 'A') {
        log(`Link clicked: ${event.target.href}`);
        
        // For same-origin links that don't open in a new tab/window
        if (event.target.origin === window.location.origin && 
            !event.target.target) {
            // We can try to log before navigation, but keep in mind
            // that the request might not complete before the page unloads
            logNavigationToServer(event.target.href, window.location.href);
        }
    }
});
